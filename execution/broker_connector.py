...
def open_trade(
    ...
    """
    # Normalize alternate parameter names used in other modules
    # main.py uses keywords: volume, sl, tp — accept those too
    if volume is not None and lot_size is None:
        lot_size = volume
    if sl is not None and stop_loss is None:
        stop_loss = sl
    if tp is not None and take_profit is None:
        take_profit = tp
    ...
    stop_loss    = round(stop_loss, digits) if stop_loss is not None else 0.0
    take_profit  = round(take_profit, digits) if take_profit is not None else 0.0
    lot_size     = round(lot_size if lot_size is not None else 0.0, 2)
...
# ---------------------------------------------------------------------------
# Closed deals history (for reconciliation)
# ---------------------------------------------------------------------------

def get_closed_deal_by_ticket(ticket: int) -> Optional[dict]:
    """
    Recupera un deal chiuso dalla history di MT5 usando il position ticket.

    Parameters
    ----------
    ticket : int
        Position ticket da cercare nella history.

    Returns
    -------
    dict or None
        Dizionario con chiavi: symbol, deal_ticket, profit, close_price,
        close_time, entry_price, volume, entry_type, exit_type
    """
    # MT5 history_deals_get richiede datetime NAIVE (locale, senza timezone).
    # Passare un datetime UTC-aware causa mancata corrispondenza e restituisce
    # zero risultati — motivo principale per cui i trade non venivano trovati.
    # Finestra: 400 giorni per coprire trade storici del conto demo.
    from datetime import datetime, timedelta
    since_date = datetime.now() - timedelta(days=400)

    deals = mt5.history_deals_get(since_date, datetime.now())
    if deals is None or len(deals) == 0:
        logger.warning("get_closed_deal_by_ticket: nessun deal nella history MT5 (ticket=%d)", ticket)
        return None

    # FIX: rimuoviamo il filtro sul magic number per trovare anche i trade
    # aperti dal vecchio bot (magic=0) o da altre versioni del software.
    # Identifichiamo la posizione solo tramite position_id.
    for deal in deals:
        if deal.position_id == ticket and deal.entry == mt5.DEAL_ENTRY_OUT:
            return {
                ...
                "volume":          deal.volume,
                "deal_type":       "SELL" if deal.type == mt5.DEAL_TYPE_SELL else "BUY",
                "entry_type":      deal.entry,
            }

    logger.debug(
        "get_closed_deal_by_ticket: ticket=%d non trovato tra %d deal (400gg)",
        ticket, len(deals)
    )
    return None


def get_closed_deals_since(days_ago: int = 60) -> list[dict]:
    """
    Recupera tutti i deal chiusi dal bot negli ultimi N giorni.

    Parameters
    ----------
    days_ago : int
        Numero di giorni indietro da cercare (default 60).

    Returns
    -------
    list of dict
        Lista dei deal chiusi con chiavi: symbol, deal_ticket, position_ticket,
        profit, close_price, close_time, volume.
    """
    from datetime import datetime, timedelta, timezone
    
    since_date = datetime.now(tz=timezone.utc) - timedelta(days=days_ago)
    deals = mt5.history_deals_get(since_date)
